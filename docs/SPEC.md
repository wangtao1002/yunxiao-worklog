...
const DEFAULTS = {
  ...
  prefs: {
    ...
    hoursBasis: 'estimated',   // 统计展示与字段裁剪口径：'estimated' | 'actual' | 'both'
...
**概览卡**：第一行是任务数 / 工时 / 日均工时 / 逾期率等常规统计；预计、实际及两者偏差按 `hoursBasis` 裁剪。
...
**未填工时告警**（`prefs.warnMissingEst`，默认开）：按 `hoursBasis` 检查预计、实际或两者，判据均为未取消且对应工时 `<= 0`
...
**日历热力图**：
- 按周排列（周一起），每格显示日期 + 工时数
- 颜色深浅按 `hoursBasis` 使用预计或实际工时；两者模式可切换；工作日不足 `dailyTargetHours` 的格子加醒目描边
...
- 每日标准工时（number，默认 8）
- 默认归集口径、任务状态范围、达标工时口径、默认时间范围、统计展示口径
